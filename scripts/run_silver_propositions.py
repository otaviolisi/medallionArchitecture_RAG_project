"""Run the propositions silver transformation and load to Supabase.

Processes only delta files not yet loaded into silver (append-only).

Usage:
    python scripts/run_silver_propositions.py
    python scripts/run_silver_propositions.py --dry-run
"""
...
from bussola.silver.propositions import transform

log = get_logger("bussola.silver.run_propositions")

SILVER_STATE_FILE = Path("data/silver/_state.json")
SILVER_CONFIG_FILE = Path("config/silver.yaml")
ENDPOINT_KEY = "propositions"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Propositions silver layer loader")
    ...
    if not new_files:
        log.info("No new delta files to process for propositions.")
        return
    ...
    log.info("Propositions silver complete.")
